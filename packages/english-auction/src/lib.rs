...
const ONE_DAY: u64 = 86400; // 1 day = 24 hours * 60 minutes * 60 seconds = 86400 seconds.
...
impl EnglishAuction {
    ...
    pub fn start(&mut self) -> Result<(), EnglishAuctionError> {
        ...
        let nft = IERC721::new(self.nft_address.get());
        // Get the NFT ID.
        ...
        match result {
            ...
            Ok(_) => {
                ...
                self.end_at.set(U256::from(block::timestamp() + 7 * ONE_DAY));
                // Log the start event.
                evm::log(Start {});
                Ok(())
            },
    ...
    pub fn end(&mut self) -> Result<(), EnglishAuctionError> {
        ...
        self.ended.set(true);
        let nft_contract_address = self.nft_address.get();
